...
res=s.get(url)
resp=str(res)
if resp[resp.find('[')+1:resp.find(']')]=='200':
    print('Input Permutations Extracted')
else:
    print('Input Permutation Extraction Unsuccessful')
s.close()
...
print('Unrequired Trip Combinations Removed')
...
for k in range(0,len(iterations)):
    ...
    lead_time=(trip_com-dt.date.today()).days
...
df['EMAIL']=returns[3]
df['MOBILE']=returns[4]
print('Lead Traveller Data Created')
...
print('Input Permutations File Created')
